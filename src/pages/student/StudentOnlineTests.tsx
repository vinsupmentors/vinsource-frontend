...
import { Loader2, ClipboardList, Clock, AlertTriangle, CheckCircle2, XCircle } from 'lucide-react';
...
export default function StudentOnlineTests() {
  ...
  const load = () => {
    setLoading(true);
    api.get('/api/student-portal/online-tests').then((r) => setReleases(r.data.data || [])).finally(() => setLoading(false));
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold">Online Tests</h1>
        <p className="text-sm text-muted-foreground mt-1">Once started, each test runs for a fixed duration and submits automatically if you switch tabs.</p>
      </div>

      {releases.length === 0 ? (
        <div className="bg-card rounded-xl border p-10 text-center text-muted-foreground text-sm">No tests have been activated for you yet.</div>
      ) : (
        <div className="space-y-3">
          {releases.map((r) => {
            ...
            return (
              <div key={r.releaseId} className="bg-card rounded-xl border p-4 flex items-center justify-between gap-3 flex-wrap">
                <div>
